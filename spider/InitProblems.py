...
def init_problems_multithreading():
    ...
    for oj_name in config.oj_list:
        thread_list = []
        for problem_list in get_problem_lists(oj_name):
            th = threading.Thread(target=init_problems_from_problem_list, args=(problem_list,))
            th.start()
...
def init_problems_from_problem_list(problem_list):
    for problem in problem_list:
        if problem['allowSubmit'] and db["remoteOJs"].find({
            ...
            _problem['description_url'] = "https://vjudge.net" + problem_html.iframe['src']
...
def save_problem(problem):
    ...
    if db["problem"].find({
        ...
        print_info("更新 {}-{} {}".format(problem['soj'], problem['sid'], problem["title"]))
    else:
        # 添加新题目
        problem["totalsm"] = 0
        problem["totalac"] = 0
        db["problem"].insert_one(problem)
        print_info("添加 {}-{} {}".format(problem['soj'], problem['sid'], problem["title"]))

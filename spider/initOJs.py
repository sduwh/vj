...
def init_remote_ojs():
    ...
    try:
        response = requests.get(url=config.remote_oj_url, proxies=config.proxy, timeout=config.timeout)
...
def save_oj_to_db(remote):
    ...
    with pymongo.MongoClient(config.dbhost) as client:
        ...
        for oj_name in remote:
            print_info("开始处理 oj: {}".format(oj_name))
            # 检查数据库中支持的 OJ
            if not db["OJ"].find({"soj": oj_name}).count():
                db["OJ"].insert({"soj": oj_name})
                print_info("添加 oj {}".format(oj_name))
            for key, value in remote[oj_name].get("languages").items():
                if db["remoteOJs"].find({
                    ...
                    print_info("更新 {} 方法 {}: {}".format(oj_name, key, value))
                else:
                    # 添加
                    db["remoteOJs"].insert({
                        "soj": oj_name,
                    ...
                    print_info("添加 {} 方法 {}: {}".format(oj_name, key, value))
